unit UTextSearchZeos;

interface

uses
  SysUtils, Classes, Controls, StdCtrls, ExtCtrls, DBGrids, Types, Windows, Variants,
  UGridForm, DB, ZConnection, ZAbstractRODataset, ZDataset, Messages, Forms;

type
  TTextSearchZeos = class(TEdit)
  private
      FormGrid: TGridForm;
      ...
      procedure SetDataBase(D: TZConnection);
  ...
  published
      ...
      property zDataBase:        TZConnection read FDataBase        write SetDataBase;
...
procedure Register;
begin
  RegisterComponents('Éderson', [TTextSearchZeos]);
end;
...
constructor TTextSearchZeos.Create(AOwner: TComponent);
begin
   inherited Create(AOwner);
   DataSet           := TZReadOnlyQuery.Create(Self);
...
destructor TTextSearchZeos.Destroy;
begin
...
procedure TTextSearchZeos.Change;
var
...
begin
   ...
   begin
      try
         ...
         DataSet.ParamByName('text_to_search').AsString := Trim(Self.Text) + '%';
         DataSet.Open;
...
procedure TTextSearchZeos.DoEnter;
begin
   HasFocus := True;
   inherited;
end;

procedure TTextSearchZeos.DoExit;
begin
   HasFocus := False;
   HideGrid;
   inherited;
...
procedure TTextSearchZeos.KeyPress(var Key: Char);
begin
   if Key = #13 then
   begin
...
procedure TTextSearchZeos.KeyDown(var Key: Word; Shift: TShiftState);
begin
...
procedure TTextSearchZeos.CreateGrid;
begin
...
procedure TTextSearchZeos.ShowGrid;
...
procedure TTextSearchZeos.HideGrid;
begin
   if FormGrid = nil then
      CreateGrid;
...
// popup grid
procedure TTextSearchZeos.RecordChosen(Sender: TObject);
begin
...
procedure TTextSearchZeos.DoOnSelectRecord(Sender: TObject);
begin
...
procedure TTextSearchZeos.SetDataBase(D: TZConnection);
begin
   FDataBase          := D;
   DataSet.Connection := D;
end;

procedure TTextSearchZeos.GenerateNewSQLSelect;
var
   SelectFields: string;
   i:            integer;
begin
...
procedure TTextSearchZeos.SetGridFieldNames(G: TStrings);
begin
   FGridFieldNames.Assign(G);
end;

procedure TTextSearchZeos.SetGridFieldWidths(G: TStrings);
begin
   FGridFieldWidths.Assign(G);
end;

procedure TTextSearchZeos.SetGridFieldFormats(G: TStrings);
begin
...
procedure TTextSearchZeos.PutData(NewText: string; NewSelectedKey: variant);
begin
...
procedure TTextSearchZeos.ClearData;  // What does this mean? :P
begin
...
function TTextSearchZeos.FieldValue(FieldName: string): variant;
begin
